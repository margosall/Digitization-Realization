...
{
    uint8_t *view_data = new uint8_t[width * height];
    int16_t *view_data_min = new int16_t[width];
    int16_t *view_data_max = new int16_t[width];
    //
    ...
    int16_t x_step = (int16_t)(width) / (int16_t)len;
    int16_t y_step = (int16_t)(height - 1) / (max - min);
    int16_t data_min = std::numeric_limits<int16_t>::max();
    int16_t data_max = std::numeric_limits<int16_t>::min();
    int min_pos = 0;
    int max_pos = 0;
    ...
    for (int i = 0 ; i < len ; i++) {
        ...
        ESP_LOGD("view", "for i=%i, x_pos=%i,  max=%i, min=%i, data=%i", i, x_pos, view_data_min[x_pos], view_data_max[x_pos], data[i]);
        if (data[i] > data_max) {
            data_max = data[i];
            max_pos = i;
        }
    ...
    ESP_LOGI("view", "Data min[%i] = %i, Data max[%i] = %i", min_pos, data_min, max_pos, data_max);
    ESP_LOGD("view", "y_step = %i", y_step);
    for (int x = 0 ; x < width ; x++) {
        int y_count = (view_data_max[x] - view_data_min[x]) * y_step + 1;
        ESP_LOGD("view", "For x= %i y_count=%i  ,min =%i, max=%i, ... ", x, y_count, view_data_min[x], view_data_max[x]);
        for (int y = 0 ; y < y_count ; y++) {
            int y_pos = (max - view_data_max[x]) * y_step + y;
            ESP_LOGD("view", " %i, ", y_pos);
            view_data[y_pos * width + x] = view_char;
    ...
    //     if (data[i] <= min) y_pos = height-1;
    //     view_data[y_pos*width + x_pos] = view_char;
    //     printf("For data[%i]=%i, x_pos%i, y_pos=%i\n", i, data[i], x_pos, y_pos);
    // }
    // printf("\n");
    printf(" ");
    for (int x = 0 ; x < width ; x++) {
    ...
    printf("\n");
    ESP_LOGI("view", "Plot: Length=%i, min=%i, max=%i", len, min, max);
    delete view_data;
    delete view_data_min;
    delete view_data_max;
}
