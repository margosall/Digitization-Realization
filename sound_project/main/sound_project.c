...
void readSignal(void *soundInput) {
    ...
    for(;;) {
        bytesRead = raw_stream_read(soundSettings->raw_read, (char *)soundSettings->buffer, AUDIOCHUNKSIZE * sizeof(int16_t));
        ...
        // for (counter = 0; counter < AUDIOCHUNKSIZE; counter++) {
        //     y_ct[counter * 2 + 0] = soundSettings->buffer[counter];
        //     y_ct[counter * 2 + 1] = 0;
        //     printf("%hi ", y_ct[counter * 2 + 0]);
        // }

        // dsps_fft2r_fc32(y_ct, AUDIOCHUNKSIZE);
        // dsps_bit_rev_fc32(y_ct, AUDIOCHUNKSIZE);
        // dsps_cplx2reC_fc32_ansi(y_ct, AUDIOCHUNKSIZE);
        // for (counter = 0; counter < AUDIOCHUNKSIZE/2; counter++) {
        //     y_ct[counter] = 10 * log10(y_ct[counter * 2 + 0] * y_ct[counter * 2 + 0] + y_ct[counter * 2 + 1] * y_ct[counter * 2 + 1])/AUDIOCHUNKSIZE;
        //     printf("%f ", y_ct[counter]);
        // }
        // printf("%d\n", bytesRead);
        // printf("\n");
    }
